...
/**
 * Create a `DrainQueue`. The chain starts resolved so the first `awaitDrain()`
 * settles immediately once a `"drain"` fires; each waiter appends itself to the
 * chain, and a per-waiter `.catch` keeps the chain alive even if that waiter
 * rejects (e.g. the socket errored out while it was queued).
 */
